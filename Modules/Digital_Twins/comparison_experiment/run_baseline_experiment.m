function timing = run_baseline_experiment(output_dir)
%RUN_BASELINE_EXPERIMENT Run the paced deterministic comparison model.
...
model_name = 'DT_enviroment_state_machine_baseline_instrumented';
...
set_param(model_name, 'EnablePacing', 'on', 'PacingRate', '1', 'StopTime', '120');

sampler = start_cpu_sampler('baseline', output_dir);
wall_clock = tic;
...
data = normalize_experiment_output(simulation_output, false);
writetable(data, fullfile(output_dir, 'baseline_data.csv'));

timing = make_timing( ...
    'Deterministic state machine', ...
    120.0, wall_time_s, matlab_cpu_s, model_path);
save(fullfile(output_dir, 'baseline_timing.mat'), 'timing');
